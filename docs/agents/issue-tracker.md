...
- **Simulators.** All four devices ("YourTube 1" through "YourTube 4") are pooled via `scripts/simpool.sh`, which discovers them by name prefix. Tests and `-seedFixtures` drives take a plain `acquire <name>`, which never hands out the signed-in-store device. "YourTube 1" holds that store (its UDID is `STORE_UDID` at the top of `simpool.sh`); a drive that genuinely needs real data takes a lock on it with `acquire --store <name>`, so a wave carries at most one issue that drives it. Install and test upgrade in place; `xcrun simctl uninstall` or `erase` on that device wipes the store, so agents never run them.
...
- **The `/run-issues` and `/take-issue` skills moved out of this repo** (PR #121, to user-level skill files) and no longer carry device names to keep in sync; this file and `README.md` are the source of truth for the simulator pool.
